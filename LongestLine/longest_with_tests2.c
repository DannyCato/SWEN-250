...
#define TRUE (1) // Truth values to make logic more readable
#define FALSE (0) // ^^^
...
int main( int argc, char *argv[] ) {		// LLK addition for unit testing
	...
	int length = 0; // Current Shortest
	char longest[MAXLINE + 1]; // Current Longest Line
...
int readline( char s[], int max_chars ) {
	int character, len;
	for (len = 0; len < MAXLINE; len++) // fills s[] with the characters stdin until a \n or EOF is reached
	{
		character = getchar();
		if (character == '\n') 
		{
...
void copy( char to[], char from[] ) {
	...
	//	Do NOT use pointers.
	int i = 0;
	do		// in do-while form to make from[i - 1] condition applicable
	{
		to[i] = from[i];
		i++;
	}
	while (from[i - 1] != '\0'); // copies values from from[] to to[] until "\0" was the last value inputted
}
