...
int readline( char s[], int max_chars ) {
	int character, len = 0;
	for (character = getchar(); len <= MAXLINE; character = getchar()) // fills s[] with the characters of stdin until a \n or EOF is reached as long as the line is not too long
	{
		// printf("%c", character);
		if (character == '\n') 
		{
