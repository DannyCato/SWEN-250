...
int *create_array_of_products( int *p_first, int *p_second, int number_of_entries )
{
	...
	int *ret = malloc( sizeof(int) * number_of_entries ) ;
	for ( int i = 0 ; i < number_of_entries ; i++ )
	{
		ret[i] = p_first[i] * p_second[i] ;
	}
	return ret ;	// Fix this -- this value causes all unit tests to fail.
}
...
int get_total_and_free( int *p_array, int number_of_entries )
{
	...
	free( p_array ) ;
	return ret ;		// Fix this -- causes all unit tests to fail.
}
...
{
	// is this a good choice for initialization?
